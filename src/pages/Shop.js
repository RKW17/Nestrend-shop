...
const Shop = function ({
  ...
  const downloadReceipt = () => {
    ...
    const itemDetails = cart
      .map(
        (i) =>
          `${i.name} ${i.chosenSize ? `(Size: ${i.chosenSize})` : ""} ${i.selColor ? `(Color: ${i.selColor})` : ""} (x${i.qty}) - ${formatGHS(i.price * i.qty)}`,
      )
      .join("\n");
    const receiptText = `
  ...
  const sendOrderToManager = async () => {
    ...
    try {
      const itemDetails = cart
        .map(
          (i) =>
            `${i.name} ${i.chosenSize ? `(Size: ${i.chosenSize})` : ""} ${i.selColor ? `(Color: ${i.selColor})` : ""} (x${i.qty})`,
        )
        .join("\n");
                  ...
                  <div key={idx} style={cartItemStyle}>
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        ...
                        {item.chosenSize && `| Size: ${item.chosenSize}`}
                      </p>
                    </div>
                    <Trash2
                      size={18}
