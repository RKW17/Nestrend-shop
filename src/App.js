              ...
              {categories.map((cat) => (
                <div key={cat.name}>
                  <div onClick={() => setExpandedCat(expandedCat === cat.name ? null : cat.name)} style={menuItem}>
                    <span onClick={(e) => { e.stopPropagation(); handleCategoryChange(cat.name); }}>
                  ...
                  {expandedCat === cat.name && subcategoryMap[cat.name] && (
                    <div style={subMenuBox}>
                      <p onClick={() => handleCategoryChange(cat.name)} style={subMenuItem}>
                        ALL {cat.name.toUpperCase()}
                      </p>
                      {subcategoryMap[cat.name].map((sub) => (
                        <p key={sub} onClick={() => { setSubcategory(sub); setCategory(cat.name); setIsMenuOpen(false); }} style={subMenuItem}>
                          {sub.toUpperCase()}
                        </p>
                      ))}
                    </div>
...
const tagline = { color: "#D4AF37", fontSize: "11px", letterSpacing: "4px", fontWeight: "900", margin: "8px 0 0" };
const subTagline = { fontSize: "10px", letterSpacing: "2px", opacity: 0.8 };
const stickyNav = { padding: "15px 30px", background: "#fff", position: "sticky", top: 0, zIndex: 1000, boxShadow: "0 4px 20px rgba(0,0,0,0.1)", display: "flex", alignItems: "center" };
const navContainer = { display: "flex", flexDirection: "column", alignItems: "center", flex: 1, gap: "10px" };
const searchWrapper = { display: "flex", alignItems: "center", background: "#f5f5f5", padding: "8px 20px", borderRadius: "50px", width: "65%" };
const longSearchField = { background: "none", border: "none", marginLeft: "10px", width: "100%", outline: "none", fontWeight: "600" };
const catCenter = { display: "flex", gap: "12px", flexWrap: "wrap", justifyContent: "center" };
const catBubble = { background: "none", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: "4px" };
const catImgWrapper = { width: "45px", height: "45px", borderRadius: "50%", padding: "2px", border: "2px solid transparent", overflow: "hidden" };
const catImgStyle = { width: "100%", height: "100%", borderRadius: "50%", objectFit: "cover" };
const catLabel = { fontSize: "8px", fontWeight: "900", textAlign: "center", maxWidth: "60px" };
...
const menuDrawer = { width: "280px", background: "#fff", height: "100%", padding: "30px 20px", overflowY: "auto" };
...
const menuItem = { display: "flex", justifyContent: "space-between", padding: "12px 0", borderBottom: "1px solid #f9f9f9", fontWeight: "900", fontSize: "12px", cursor: "pointer" };
const subMenuBox = { background: "#f9f9f9", padding: "10px" };
const subMenuItem = { fontSize: "10px", fontWeight: "700", padding: "8px 0", cursor: "pointer", color: "#555" };
...
const footerSubTagline = { fontSize: "11px", letterSpacing: "3px", color: "#D4AF37", marginTop: "5px", fontWeight: "600" };
const contactBox = { display: "flex", flexDirection: "column", gap: "10px", marginTop: "20px", alignItems: "center" };
const contactItem = { display: "flex", alignItems: "center", gap: "10px", fontSize: "14px" };
const devSection = { marginTop: "30px", borderTop: "1px solid #222", paddingTop: "20px" };
